resource "aws_sfn_state_machine" "ausseabed-processing-pipeline_sfn_state_machine-ga" {
  ...
  "Next": "Resume from step"
    ...
    "Default": "Get caris version"
      ...
      ,"Upload checkpoint 3 to s3":{"Type":"Task","Resource":"arn:aws:states:::ecs:runTask.sync","ResultPath": "$.previous_step__result","Parameters":{"LaunchType":"FARGATE","Cluster":"${module.compute.aws_ecs_cluster_arn}","TaskDefinition":"${module.compute.aws_ecs_task_definition_caris-version_arn}","NetworkConfiguration":{"AwsvpcConfiguration":{"AssignPublicIp":"ENABLED","SecurityGroups":["${module.networking.aws_ecs_task_definition_caris_sg}"],"Subnets":["${module.networking.aws_ecs_task_definition_caris_subnet}"]}},"Overrides":{"ContainerOverrides":[{"Name":"app","Command.$":"$.s3_up_sync_command"}]}},"Next":"Sonar noise classifier","TimeoutSeconds":6000     }
      ,"Sonar noise classifier":{"Type":"Task","Resource":"arn:aws:states:::ecs:runTask.sync","ResultPath": "$.previous_step__result","Parameters":{"LaunchType":"FARGATE","Cluster":"${module.compute.aws_ecs_cluster_arn}","TaskDefinition":"${module.compute.aws_ecs_task_definition_caris-version_arn}","NetworkConfiguration":{"AwsvpcConfiguration":{"AssignPublicIp":"ENABLED","SecurityGroups":["${module.networking.aws_ecs_task_definition_caris_sg}"],"Subnets":["${module.networking.aws_ecs_task_definition_caris_subnet}"]}},"Overrides":{"ContainerOverrides":[{"Name":"app","Command":["-ip","52.62.84.70", "-c","powershell carisbatch --run ClassifyHIPSNoise \"file:///${var.local_storage_folder}\\GA-0364_BlueFin_MB\\GA-0364_BlueFin_MB.hips\" -v --mira-url http://arnab.rakshit^@ga.gov.au:qW1!eight@carismira.ai --level-of-detail Fine --finest-vertical-resolution 0.05m --noise-confidence-filter-threshold 70 "]}]}},"Next":"Filter Processed Depths Swath","TimeoutSeconds":60000     }
      ,"Filter Processed Depths Swath":{"Type":"Task","Resource":"arn:aws:states:::ecs:runTask.sync","ResultPath": "$.previous_step__result","Parameters":{"LaunchType":"FARGATE","Cluster":"${module.compute.aws_ecs_cluster_arn}","TaskDefinition":"${module.compute.aws_ecs_task_definition_caris-version_arn}","NetworkConfiguration":{"AwsvpcConfiguration":{"AssignPublicIp":"ENABLED","SecurityGroups":["${module.networking.aws_ecs_task_definition_caris_sg}"],"Subnets":["${module.networking.aws_ecs_task_definition_caris_subnet}"]}},"Overrides":{"ContainerOverrides":[{"Name":"app","Command":["-ip","52.62.84.70", "-c","powershell \"carisbatch --run CreateVRSurface --estimation-method RANGE --range-file ${local_storage_folder}\\depth_ranges_for_filtering.txt --range-method PERCENTILE --range-percentile 50 --input-band DEPTH --max-grid-size 64 --min-grid-size 4 --include-flag ACCEPTED \"file:///${var.local_storage_folder}\\GA-0364_BlueFin_MB\\GA-0364_BlueFin_MB.hips\" \"file:///${var.local_storage_folder}\\GA-0364_BlueFin_MB\\temp_for_filtering.csar\" ; carisbatch --run PopulateVRSurface --population-method CUBE --input-band Depth --include-flag ACCEPTED --vertical-uncertainty "Depth TPU" --horizontal-uncertainty "Position TPU" --display-bias HIGHEST --disambiguation-method DENSITY_LOCALE --cube-config-file=D:\\Bluefin\\CUBEParams_AusSeabed_2019.xml  --cube-config-name="AusSeabed_VR" \"file:///${var.local_storage_folder}\\GA-0364_BlueFin_MB\\GA-0364_BlueFin_MB.hips\" \"file:///${var.local_storage_folder}\\GA-0364_BlueFin_MB\\temp_for_filtering.csar\"  ;carisbatch  --run FilterProcessedDepths   --filter-type SURFACE --surface \"${var.local_storage_folder}\\GA-0364_BlueFin_MB\\BlueFin_2018-172_1m.csar\" --threshold-type STANDARD_DEVIATION --scalar 1.6 \"file:///${var.local_storage_folder}\\GA-0364_BlueFin_MB\\GA-0364_BlueFin_MB.hips\"\" "]}]}},"Next":"Upload checkpoint 4 to s3","TimeoutSeconds":60000     }
