...
def getLastExecutionArnIfFailed(state_machine_arn):
    response = client.list_executions(
    stateMachineArn=state_machine_arn,
    #statusFilter='RUNNING'|'SUCCEEDED'|'FAILED'|'TIMED_OUT'|'ABORTED',
    maxResults=2)
    index_previous_run = 1
    print("last Execution response: {}".format(response))
    status = response['executions'][index_previous_run]['status'] # index-0 is the current one which is always "RUNNING" so we look at index-1 which is the previous execution
    
    if status in ['FAILED']:
        return response['executions'][index_previous_run]['executionArn']
    else:
        return None
...
def parseFailureHistory(failedExecutionArn):
    ...
    if failedEvents[0]['executionFailedEventDetails']['error'] == 'States.Runtime':
        ...
        return ("Default", "{}")  # This is a failure we can't do anything about so continuing with the whatever the Default next step is.
    '''
...
if __name__ == '__main__':
    ...
    Main
    Run as: 
    python gotostate.py --stateMachineArn '<stateMachineArn>'"
    '''
    parser = argparse.ArgumentParser(description='Execution Arn of the failed state machine.')
    parser.add_argument('--stateMachineArn', dest='stateMachineArn', type=str)
    args = parser.parse_args()
    arnLastExecutionIfFailed = getLastExecutionArnIfFailed(args.stateMachineArn)
    print("last Execution arn is: {}".format(arnLastExecutionIfFailed))
    if arnLastExecutionIfFailed is not None:
        failedSMInfo = parseFailureHistory(arnLastExecutionIfFailed)
...
def lambda_handler(event, context):
    arnLastExecutionIfFailed = getLastExecutionArnIfFailed(event['stateMachineArn'])
    print("last Execution arn is: {}".format(arnLastExecutionIfFailed))
    mydict = {}
    if arnLastExecutionIfFailed is not None:
        failedSMInfo = parseFailureHistory(arnLastExecutionIfFailed)
        print("Execution had failed at state: {} with Input: {}".format(failedSMInfo[0], failedSMInfo[1]))
        mydict = {"state": failedSMInfo[0], "Input":  failedSMInfo[1]}
    else:
        mydict = {"state": "", "Input":  ""}
        print("Last execution didnot fail")
    return {
