...
if [ -z "${GEOGRAPHIC}" ] 
then
  ogr2ogr area_"$LOCALNAME_DEST".shp poly_min_"$LOCALNAME_DEST".shp -sql "SELECT *, OGR_GEOM_AREA / 1000000 AS area_km2 FROM poly_min_$LOCALNAME_DEST"
else
  ogr2ogr area_"$LOCALNAME_DEST".shp poly_min_"$LOCALNAME_DEST".shp -sql "SELECT *, OGR_GEOM_AREA AS area_dg2 FROM poly_min_$LOCALNAME_DEST"
fi
